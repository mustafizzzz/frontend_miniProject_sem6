...
const OralSocketPage = () => {
  ...
  useEffect(() => {
    const fetchTestDetails = async () => {
      setIsEnding(true);
      setBackDropMessage("Lodaing test please wait...");
      try {
        const res = await axios.post(`https://mood-lens-server.onrender.com/api/v1/test/view_test_details`, { test_id });
        console.log("current Test:", res.data);

        setTestData(res.data);
        setRemainingTime(res.data.test.maxDuration * 60); // assume duration in minutes
  ...
  useEffect(() => {
    ...
    if (location.pathname.startsWith("/test-socket") && testData) {
  ...
  }, [location.pathname, testId, testData]);
  ...
  //generate proctoring
  const generateProctoringSummary = () => {
    if (proctorViolations.length === 0) {
      return "No suspicious activity detected during the exam."
    }

    const tabSwitches = proctorViolations.filter((v) => v.type === "tab_switch").length
    const screenResizes = proctorViolations.filter((v) => v.type === "screen_resize").length

    return `Detected ${tabSwitches} tab switches and ${screenResizes} screen resizes during the exam. ${tabSwitches + screenResizes > 3
      ? "This level of activity is concerning."
      : "This level of activity is within acceptable limits."
      }`
  }
  // Handle proctoring violations
  const handleProctorViolation = (type, description) => {
    const violation = {
      type,
      description,
  ...
  const handleEndExam = async () => {
    ...
    try {
      ...
      setTimeout(async () => {
        const videoUrl = await uploadSingleVideoToFirebase(storage, testId, currentUser.userName);
        setBackDropMessage("Evaluating your test,please wait...");

        socket.emit("end_test", {
          test_id: test_id || DEFAULT_TEST_ID,
          student_id: currentUser.hostId || 5123,
          videoUrl,
        });
      }, 4000);

      socket.on('evaluation', (evaluationResponse, proctoringSummary) => {
        setEvaluationData({
          ...evaluationResponse,
          proctoringSummary,
        });
      ...
      <div className="container">
        ...
        <div className="card mx-auto main-container">
          ...
          <div className="card-header bg-primary text-white d-flex justify-content-between align-items-center">
            ...
            <div className="voice-selector d-flex align-items-center gap-2" style={{ width: 'auto' }}>
              <select
                id="voiceSelect"
        ...
        {/* Proctoring Service */}
        <ProctorService onViolation={handleProctorViolation} isExamActive={isExamActive} />
        ...
        <Dialog open={dialogOpen} onClose={() => { }} fullWidth maxWidth="sm">
          ...
          <DialogContent>
            <div className="mb-4">
              <h5 className="text-primary">Academic Evaluation</h5>
              <p>
                <strong>Feedback:</strong> {evaluationData?.feedback}
              </p>
              <p>
                <strong>Overall Summary:</strong> {evaluationData?.summary}
              </p>
              <p>
                <strong>Copied:</strong> {evaluationData?.copied || "True"}
              </p>
              <p>
                <strong>Marks:</strong> {evaluationData?.marks} out of {testData?.maxMarks || "50"}
              </p>
            </div>

            <div className="mb-3">
              <h5 className="text-danger">Proctoring Report</h5>
              <p>{evaluationData?.proctoringSummary}</p>
                ...
                <div className="mt-3">
                  ...
                  <div className="table-responsive">
                    <table className="table table-sm table-bordered">
                      ...
                      <tbody>
                        {proctorViolations.map((violation, index) => (
                          <tr key={index}>
                            <td>{new Date(violation.timestamp).toLocaleTimeString()}</td>
                            <td>{violation.type === "tab_switch" ? "Tab Switch" : "Screen Resize"}</td>
          ...
          <DialogActions>
            <Button onClick={() => (window.location.href = "/dashboard/home")} color="primary">
              Close
            </Button>
          </DialogActions>
        </Dialog>
